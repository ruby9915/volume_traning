...
fixture(app/client/auth_client/db)는 conftest.py, 일반 함수는 이 모듈.
"""
...
def add_session(db: sqlite3.Connection, date: str) -> int:
    cur = db.execute("INSERT INTO workout_session (date) VALUES (?)", (date,))
    db.commit()
    return cur.lastrowid


def add_set(db, session_id, exercise_id, weight, reps, warmup=0, note=None) -> None:
    """DB 직접 삽입 (API 우회) — set_index는 세션 내 다음 번호."""
    idx = db.execute(
        "SELECT COALESCE(MAX(set_index), 0) + 1 FROM workout_set WHERE session_id = ?",
        (session_id,),
    ).fetchone()[0]
    db.execute(
        "INSERT INTO workout_set (session_id, exercise_id, set_index, weight_kg, reps, is_warmup, note)"
        " VALUES (?, ?, ?, ?, ?, ?, ?)",
        (session_id, exercise_id, idx, weight, reps, warmup, note),
    )
    db.commit()


def add_bodyweight(db: sqlite3.Connection, date: str, kg: float) -> None:
    db.execute("INSERT INTO body_weight_log (date, weight_kg) VALUES (?, ?)", (date, kg))
    db.commit()
