// §3.7A 소급 적용 흐름 — 기록(Log)·세션 상세(SessionDetail) 공용 훅.
//
// 카드 의도를 바꿨을 때 이미 저장된 같은 종목 세트를 어떻게 할지는 설정(intentBackfill)이 정한다:
...
export function useIntentBackfill<T>(run: (target: T) => Promise<void>) {
  const intentBackfill = useAppStore((s) => s.intentBackfill);
  const setIntentBackfill = useAppStore((s) => s.setIntentBackfill);
  ...
  const request = async (target: T): Promise<void> => {
    if (intentBackfill === "never") return;
    if (intentBackfill === "always") {
      await run(target);
      return;
    }
    setPrompt(target);
  ...
  const resolve = async (apply: boolean, remember: boolean): Promise<void> => {
    ...
    if (remember) setIntentBackfill(apply ? "always" : "never");
