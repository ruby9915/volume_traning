// §10.2 소급 적용 확인 다이얼로그 — 설정 "물어보기"일 때만 열린다.
// "다시 묻지 않기" 체크 후 적용/적용 안 함을 고르면 호출측(useTargetBackfill)이 그 선택을
...
export default function TargetBackfillModal({
  open,
  count,
  code,
  busy = false,
  ...
  return (
    ...
    >
      <p className="text-sm text-muted">
        이미 저장된 이 종목 {count}세트의 타겟을{" "}
        <span className="font-semibold text-text">{nameOf(code)}</span>(으)로 변경합니다.
      </p>
