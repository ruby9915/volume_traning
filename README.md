...
웨이트 트레이닝 볼륨(중량×횟수×세트)을 기록·분석하는 반응형 웹앱. v2(2026-09-08)부터 지인 다중 사용자(아이디/비밀번호 가입), 세트 타겟 부위 3단계 분류, 내장 종목 라이브러리 391종 + 즐겨찾기, 머신 카탈로그, 읽기 전용 관리자 페이지를 갖는다 (SETTING.MD §10). v2.1(2026-09-10): 고급 분석 6종(분할 실행 점검·볼륨 추세/ACWR·관여 근육 분배(간접 볼륨)·rep-max·피로 곡선·강도 존)은 **웜업 아닌 세트가 기록된 주가 4주 이상**인 사용자에게만 열린다 — 그 전엔 대시보드에 잠금 카드만 보이고 API는 403 (SETTING.MD §11).
v2.3(2026-09-15): 이력·분석 탭 병합, 프로필 탭(표시 이름·소개·아바타·공개 설정), 아이디 검색으로 친구 요청→수락, 친구끼리 기록·분석·체중 읽기 공유, 머신 검색 부위 칩, 새 버전 안내 배너 (SETTING.MD §13).
